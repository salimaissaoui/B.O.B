import { pixelArt, pixelArtOptimized } from '../../src/operations/pixel-art.js';
...
describe('pixelArtOptimized operation', () => {
  test('produces same blocks as basic pixelArt', () => {
    const step = {
      base: { x: 0, y: 0, z: 0 },
      facing: 'south',
      grid: [
        ['red_wool', 'red_wool', 'blue_wool'],
        ['green_wool', '', 'yellow_wool']
      ]
    };
    
    const basicBlocks = pixelArt(step);
    const optimizedBlocks = pixelArtOptimized(step);
    
    // Should produce same number of blocks
    expect(optimizedBlocks.length).toBe(basicBlocks.length);
    
    // Sort and compare
    const sortFn = (a, b) => a.x - b.x || a.y - b.y || a.z - b.z;
    expect(optimizedBlocks.sort(sortFn)).toEqual(basicBlocks.sort(sortFn));
  });
});
